...
const Profile = (props) => {
  ...
  const handleSubmit= async(e)=>{
    e.preventDefault();
    let res = await axios.put(`https://blogapp-jtdv.onrender.com/users/update/${userDetails._id}`,details);
    let data = res.data
    console.log(data)
    if(data.success){
  ...
  const handleCoverChanger=(e)=>{
   ...
   reader.onload=async()=>{
     ...
     let res = await axios.put(`https://blogapp-jtdv.onrender.com/users/update/${userDetails._id}`,{coverPic:reader.result});
    let data = res.data
    console.log(data)
    if(data.success){
      props.getUserDetails()
  ...
  const handleProfileChanger=(e)=>{
    ...
    reader.onload=async()=>{
      ...
      let res = await axios.put(`https://blogapp-jtdv.onrender.com/users/update/${userDetails._id}`,{profilePic:reader.result});
     let data = res.data
     console.log(data)
     if(data.success){
       props.getUserDetails()
...
<div className=" mx-auto w-[90%] bg-white shadow-xl rounded-lg text-gray-900">
  <div className="rounded-t-lg h-64 overflow-hidden relative">
    <div className='absolute right-7 bottom-6 cursor-pointer'>
  ...
  <div className='absolute right-0 top-1 cursor-pointer'>
    ...
    <img className="object-cover object-center rounded-full h-32" src={userPics.profilePic? userPics.profilePic:userDetails.profilePic} alt="Woman looking front" />
  </div>
...
<div className='bg-red-600 p-10 w-[40%] rounded-lg'>
<label className='w-[100px] mt-2 inline-block ' htmlFor="">UserName</label>
<input onChange={handleInputChanger} name='name' className=' px-3 py-1 border-2 border-gray-300  outline-none rounded-md'value={details.name} type="text" /> <br />
<label className='w-[100px] mt-2 inline-block' htmlFor="">Email</label>
<input className='px-3 py-1 border-2 border-gray-300 outline-none rounded-md' name='email' disabled value={details.email} type="email" /><br />
<label className='w-[100px] mt-2 inline-block' htmlFor="">Password</label>
<input onChange={handleInputChanger} name='password' className='px-3 py-1 border-2 border-gray-300 outline-none rounded-md'value={details.password} type="password" /><br />
<label className='w-[100px] mt-2 inline-block' htmlFor="">Bio</label>
<input onChange={handleInputChanger} name='bio' className='px-3 py-1 border-2 border-gray-300 outline-none rounded-md' value={details.bio} type="text" /><br />
<button onClick={handleSubmit} className='bg-green-800 w-full mt-4 text-white rounded-md px-3 py-2 hover:bg-green-950'>Update Details</button>
</div>
<div className='w-[60%]'>
